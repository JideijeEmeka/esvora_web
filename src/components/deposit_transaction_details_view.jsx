...
const DepositTransactionDetailsView = ({ deposit, onClose, elevated }) => {
			...
			<div className={`fixed right-0 top-0 bottom-0 w-full max-w-[480px] bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-in-out overflow-y-auto ${elevated ? 'z-[70]' : 'z-50'}`}>
				<div className='p-6'>
					<div className='flex items-center justify-between mb-6'>
						<h2 className='text-[20px] font-semibold text-gray-900'>Deposit details</h2>
					...
					<div className='space-y-4'>
						<div>
							<h3 className='text-[14px] font-semibold text-gray-900 mb-2'>Deposit information</h3>
							<dl className='space-y-2 text-[14px]'>
								{d.description && <div className='flex justify-between'><dt className='text-gray-500'>Description</dt><dd className='font-medium'>{d.description}</dd></div>}
								{d.gateway && <div className='flex justify-between'><dt className='text-gray-500'>Gateway</dt><dd className='font-medium'>{d.gateway}</dd></div>}
								{d.reference && <div className='flex justify-between'><dt className='text-gray-500'>Reference</dt><dd className='font-medium'>{d.reference}</dd></div>}
							</dl>
						</div>
						...
						{(d.created_at || d.completed_at) && (
							<div>
								...
								<p className='text-[14px] text-gray-600'>{d.completed_at || d.created_at}</p>
							</div>
						)}
					</div>
				</div>
...
export default DepositTransactionDetailsView
