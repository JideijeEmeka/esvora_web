...
const BuyView = () => {
	...
	const allProperties = useMemo(() => {
		...
		return normalized.filter((p) => {
			...
			return pType === 'sales' || pType.includes('sales')
		})
	}, [apiProperties])

	const fetchSalesProperties = (filters) => {
		if (!filters || Object.keys(filters).length === 0) {
			propertyController.filterProperties({ property_type: 'sales' }, { onError: () => {} })
			return
		}
		setIsFilterLoading(true)
		const done = () => setIsFilterLoading(false)
		const params = { property_type: 'sales' }
	...
	const handleSearchClick = () => {
		if (searchQuery.trim()) {
			...
			propertyController.filterProperties(
				{ search: searchQuery.trim(), property_type: 'sales' },
	...
	const handleFilterApply = (filters) => {
		if (!filters || Object.keys(filters).length === 0) {
			...
			fetchSalesProperties(null)
		} else {
			setActiveFilters(filters)
			fetchSalesProperties(filters)
		}
	}
	...
	const handleRemoveFilter = (filterKey) => {
		...
		fetchSalesProperties(newFilters)
	}
	...
	return (
		<>
			...
			<div className='pb-10 px-6 md:px-16 lg:px-20 mt-10'>
				...
				<div className='mb-12'>
					<div className='flex items-center justify-between mb-6'>
						<h2 className='text-[24px] font-semibold text-gray-900'>Properties for sale</h2>
