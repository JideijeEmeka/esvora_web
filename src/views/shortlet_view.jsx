...
const ShortletView = () => {
	...
	const allProperties = useMemo(() => {
		...
		return normalized.filter((p) => {
			...
			return pType === 'shortlet' || pType.includes('shortlet')
		})
	}, [apiProperties])

	const fetchShortletProperties = (filters) => {
		if (!filters || Object.keys(filters).length === 0) {
			propertyController.filterProperties({ property_type: 'shortlet' }, { onError: () => {} })
			return
		}
		setIsFilterLoading(true)
		const done = () => setIsFilterLoading(false)
		const params = { property_type: 'shortlet' }
	...
	const handleSearchClick = () => {
		if (searchQuery.trim()) {
			...
			propertyController.filterProperties(
				{ search: searchQuery.trim(), property_type: 'shortlet' },
	...
	const handleFilterApply = (filters) => {
		if (!filters || Object.keys(filters).length === 0) {
			...
			fetchShortletProperties(null)
		} else {
			setActiveFilters(filters)
			fetchShortletProperties(filters)
		}
	}
	...
	const handleRemoveFilter = (filterKey) => {
		...
		fetchShortletProperties(newFilters)
	}
	...
	return (
		<>
					...
					<div className='flex flex-col md:flex-row items-center border border-gray-300 px-4 py-3 bg-white rounded-full gap-4 w-full max-w-4xl max-md:rounded-md'>
						...
						<div className='flex items-center gap-2'>
							...
							<button
								...
								className='flex items-center gap-2 px-4 py-3 bg-white border border-gray-300 rounded-full hover:bg-gray-50 transition-colors cursor-pointer'
							>
			...
			<div className='pb-10 px-6 md:px-16 lg:px-20 mt-10'>
				...
				<div className='mb-12'>
					<div className='flex items-center justify-between mb-6'>
						<h2 className='text-[24px] font-semibold text-gray-900'>Affordable shortlets</h2>
				...
				<div className='mb-12'>
					<div className='flex items-center justify-between mb-6'>
						<h2 className='text-[24px] font-semibold text-gray-900'>Shortlet properties</h2>
