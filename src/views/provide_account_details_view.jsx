...
const ProvideAccountDetailsView = ({ onBack, onSuccess }) => {
	...
	const handleSubmit = () => {
		...
		walletController.addWithdrawalMethod({
			bankName,
			...
			onError: (m) => toast.error(m ?? 'Failed to add withdrawal method')
		})
	}

	const bankList = Array.isArray(banks) ? banks : []
	const displayBanks = bankList.map((b) => ({
		value: b.code ?? b.id,
...
export default ProvideAccountDetailsView
